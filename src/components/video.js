import React from "react"
import Container from "../components/container"
import videoStyles from "../styles/videoStyles.module.scss"
import navStyles from "../styles/navStyles.module.scss"

export default function Video() {
  return (
    <section className={videoStyles.section}>
      <div className={navStyles.anchor} id="video"></div>

      <Container>
        <div className={videoStyles.grid}>
          <h2>Videos</h2>

          {/* example */}
          <iframe
            width="560"
            height="315"
            src="https://www.youtube.com/embed/Y8Wp3dafaMQ"
            srcdoc="<style>*{padding:0;margin:0;overflow:hidden}html,body{height:100%}img,span{position:absolute;width:100%;top:0;bottom:0;margin:auto}span{height:1.5em;text-align:center;font:48px/1.5 sans-serif;color:white;text-shadow:0 0 0.5em black}</style><a href=https://www.youtube.com/embed/Y8Wp3dafaMQ?autoplay=1><img src=https://img.youtube.com/vi/Y8Wp3dafaMQ/hqdefault.jpg alt='Video The Dark Knight Rises: What Went Wrong? – Wisecrack Edition'><span>▶</span></a>"
            frameborder="0"
            allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture"
            allowfullscreen
            title="The Dark Knight Rises: What Went Wrong? – Wisecrack Edition"
          ></iframe>

          {/* example: responsive */}
          <div className={videoStyles.responsiveContainer}>
            <iframe
              width="560"
              height="315"
              src="https://www.youtube.com/embed/Y8Wp3dafaMQ"
              srcdoc="<style>*{padding:0;margin:0;overflow:hidden}html,body{height:100%}img,span{position:absolute;width:100%;top:0;bottom:0;margin:auto}span{height:1.5em;text-align:center;font:48px/1.5 sans-serif;color:white;text-shadow:0 0 0.5em black}</style><a href=https://www.youtube.com/embed/Y8Wp3dafaMQ?autoplay=1><img src=https://img.youtube.com/vi/Y8Wp3dafaMQ/hqdefault.jpg alt='Video The Dark Knight Rises: What Went Wrong? – Wisecrack Edition'><span>▶</span></a>"
              frameborder="0"
              allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture"
              allowfullscreen
              title="The Dark Knight Rises: What Went Wrong? – Wisecrack Edition"
            ></iframe>
          </div>

          {/* example: responsive, Joe vid */}
          <div className={videoStyles.responsiveContainer}>
            <iframe
              width="560"
              height="315"
              src="https://www.youtube.com/embed/k2-cvuGmWvc"
              srcdoc="<style>*{padding:0;margin:0;overflow:hidden}html,body{height:100%}img,span{position:absolute;width:100%;top:0;bottom:0;margin:auto}span{height:1.5em;text-align:center;font:48px/1.5 sans-serif;color:white;text-shadow:0 0 0.5em black}</style><a href=https://www.youtube.com/embed/k2-cvuGmWvc?autoplay=1><img src=https://i.ytimg.com/vi/k2-cvuGmWvc/hqdefault.jpg alt='Video The Dark Knight Rises: What Went Wrong? – Wisecrack Edition'><span>▶</span></a>"
              frameborder="0"
              allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture"
              allowfullscreen
              title="The Dark Knight Rises: What Went Wrong? – Wisecrack Edition"
            ></iframe>
          </div>

          {/* example: responsive, formatted, Joe vid */}
          <div className={videoStyles.responsiveContainer}>
            <iframe
              width="560"
              height="315"
              src="https://www.youtube.com/embed/k2-cvuGmWvc"
              srcdoc="
                <style>
                  *{
                    padding:0;
                    margin:0;
                    overflow:hidden
                  }
                  html,body{
                    height:100%
                  }
                  img,span{
                    position:absolute;
                    width:100%;
                    top:0;
                    bottom:0;
                    margin:auto
                  }
                  span{
                    height:1.5em;
                    text-align:center;
                    font:48px/1.5 sans-serif;
                    color:white;
                    text-shadow:0 0 0.5em black
                  }
                </style>
                <a href=https://www.youtube.com/embed/k2-cvuGmWvc?autoplay=1>
                  <img src=https://i.ytimg.com/vi/k2-cvuGmWvc/hqdefault.jpg alt='Video The Dark Knight Rises: What Went Wrong? – Wisecrack Edition'><span>▶</span>
                </a>"
              frameborder="0"
              allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture"
              allowfullscreen
              title="The Dark Knight Rises: What Went Wrong? – Wisecrack Edition"
            ></iframe>
          </div>

          {/* example: responsive, formatted, Joe vid w/ correct labels */}
          <div className={videoStyles.responsiveContainer}>
            <iframe
              width="560"
              height="315"
              src="https://www.youtube.com/embed/k2-cvuGmWvc"
              srcdoc="
                <style>
                  *{
                    padding:0;
                    margin:0;
                    overflow:hidden
                  }
                  html,body{
                    height:100%
                  }
                  img,span{
                    position:absolute;
                    width:100%;
                    top:0;
                    bottom:0;
                    margin:auto
                  }
                  span{
                    height:1.5em;
                    text-align:center;
                    font:48px/1.5 sans-serif;
                    color:white;
                    text-shadow:0 0 0.5em black
                  }
                </style>
                <a href=https://www.youtube.com/embed/k2-cvuGmWvc?autoplay=1>
                  <img src=https://i.ytimg.com/vi/k2-cvuGmWvc/hqdefault.jpg alt='Thumbnail'>
                  <span>▶</span>
                </a>"
              frameborder="0"
              allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture"
              allowfullscreen
              title="Project Airtime | Joe Stone"
            ></iframe>
          </div>

          {/* example: responsive, formatted, Joe vid w/ correct labels, my styling */}
          <div className={videoStyles.responsiveContainer}>
            <iframe
              width="560"
              height="315"
              src="https://www.youtube.com/embed/k2-cvuGmWvc"
              srcdoc="
                <style>
                  * {
                    padding: 0;
                    margin: 0;
                    overflow: hidden
                  }
                  img:first-child {
                    position: absolute;
                    top: calc(50vh - 36px);
                    left: calc(50vw - 36px);
                  }
                  img:nth-child(2) {
                    width: 100%;
                    height: auto
                  }
                  a {
                    display: inline-block;
                  }
                  a:focus,
                  a:hover {
                    color: white;
                    outline: 1px dotted white;
                    outline-offset: -40px;
                  }
                </style>
                <a href=https://www.youtube.com/embed/k2-cvuGmWvc?autoplay=1>
                  <img src=https://i.ytimg.com/vi/k2-cvuGmWvc/hqdefault.jpg alt='Thumbnail'>
                  <img src='data:image/png;base64,' alt='Play'>
                </a>"
              frameborder="0"
              allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture"
              allowfullscreen
              title="Project Airtime | Joe Stone"
            ></iframe>
          </div>

          {/* Joe */}
          <div className={videoStyles.responsiveContainer}>
            <iframe
              className={videoStyles.responsiveIframe}
              title="Project Airtime | Joe Stone"
              type="text/html"
              width="560"
              height="315"
              src="https://www.youtube.com/embed/k2-cvuGmWvc"
              srcDoc="
                <style>
                  * {
                    padding: 0;
                    margin: 0;
                    overflow: hidden
                  }
                  img:first-child {
                    position: absolute;
                    top: calc(50vh - 36px);
                    left: calc(50vw - 36px);
                  }
                  img:nth-child(2) {
                    width: 100%;
                    height: auto
                  }
                  a {
                    display: inline-block;
                  }
                  a:focus,
                  a:hover {
                    color: white;
                    outline: 1px dotted white;
                    outline-offset: -40px;
                  }
                </style>
                <a href='https://www.youtube.com/embed/k2-cvuGmWvc'>
                  <img src='data:image/png;base64,' alt='Play'>
                  <img src='https://i.ytimg.com/vi/k2-cvuGmWvc/maxresdefault.jpg' alt='Thumbnail'>
                </a>"
              frameBorder="0"
              allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
              allowFullScreen
              loading="lazy"
            ></iframe>
            <h3>Joe Stone</h3>
          </div>

          {/* Lucas */}
          <div className={videoStyles.responsiveContainer}>
            <iframe
              className={videoStyles.responsiveIframe}
              title="Project Airtime | Lucas"
              type="text/html"
              width="560"
              height="315"
              src="https://www.youtube-nocookie.com/embed/5jLUtWCr9fQ"
              srcDoc="
                <style>
                  * {
                    padding: 0;
                    margin: 0;
                    overflow: hidden
                  }
                  img:first-child {
                    position:absolute;
                    top: calc(50vh - 36px);
                    left: calc(50vw - 36px);
                  }
                  img:nth-child(2) {
                    width: 100%;
                    height: auto
                  }
                  a {
                    display: inline-block;
                  }
                  a:focus,
                  a:hover {
                    color: white;
                    outline: 1px dotted white;
                    outline-offset: -40px;
                  }
                </style>
                <a href='https://www.youtube-nocookie.com/embed/5jLUtWCr9fQ?autoplay=1'>
                  <img src='data:image/png;base64,' alt='Play'>
                  <img src='https://i.ytimg.com/vi/5jLUtWCr9fQ/maxresdefault.jpg' alt='Thumbnail'>
                </a>"
              frameBorder="0"
              allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
              allowFullScreen
              loading="lazy"
            ></iframe>
            <h3>Lucas</h3>
          </div>

          {/* Jason */}
          <div className={videoStyles.responsiveContainer}>
            <iframe
              className={videoStyles.responsiveIframe}
              title="Project Airtime | Jason"
              type="text/html"
              width="560"
              height="315"
              src="https://www.youtube-nocookie.com/embed/CJFugtJlC_s"
              srcDoc="
                <style>
                  * {
                    padding: 0;
                    margin: 0;
                    overflow: hidden
                  }
                  img:first-child {
                    position:absolute;
                    top: calc(50vh - 36px);
                    left: calc(50vw - 36px);
                  }
                  img:nth-child(2) {
                    width: 100%;
                    height: auto
                  }
                  a {
                    display: inline-block;
                  }
                  a:focus,
                  a:hover {
                    color: white;
                    outline: 1px dotted white;
                    outline-offset: -40px;
                  }
                </style>
                <a href='https://www.youtube-nocookie.com/embed/CJFugtJlC_s?autoplay=1'>
                  <img src='data:image/png;base64,' alt='Play'>
                  <img src='https://i.ytimg.com/vi/CJFugtJlC_s/maxresdefault.jpg' alt='Thumbnail'>
                </a>"
              frameBorder="0"
              allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
              allowFullScreen
              loading="lazy"
            ></iframe>
            <h3>Jason</h3>
          </div>

          {/* Serei */}
          <div className={videoStyles.responsiveContainer}>
            <iframe
              className={videoStyles.responsiveIframe}
              title="Project Airtime | Serei"
              type="text/html"
              width="560"
              height="315"
              src="https://www.youtube-nocookie.com/embed/rX511v45TIU"
              srcDoc="
                <style>
                  * {
                    padding: 0;
                    margin: 0;
                    overflow: hidden
                  }
                  img:first-child {
                    position:absolute;
                    top: calc(50vh - 36px);
                    left: calc(50vw - 36px);
                  }
                  img:nth-child(2) {
                    width: 100%;
                    height: auto
                  }
                  a {
                    display: inline-block;
                  }
                  a:focus,
                  a:hover {
                    color: white;
                    outline: 1px dotted white;
                    outline-offset: -40px;
                  }
                </style>
                <a href='https://www.youtube-nocookie.com/embed/rX511v45TIU?autoplay=1'>
                  <img src='data:image/png;base64,' alt='Play'>
                  <img src='https://i.ytimg.com/vi/rX511v45TIU/maxresdefault.jpg' alt='Thumbnail'>
                </a>"
              frameBorder="0"
              allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
              allowFullScreen
              loading="lazy"
            ></iframe>
            <h3>Serei</h3>
          </div>

          {/* Donna */}
          <div className={videoStyles.responsiveContainer}>
            <iframe
              className={videoStyles.responsiveIframe}
              title="Project Airtime | Donna"
              type="text/html"
              width="560"
              height="315"
              src="https://www.youtube-nocookie.com/embed/3HqPbHUSeU4"
              srcDoc="
                <style>
                  * {
                    padding: 0;
                    margin: 0;
                    overflow: hidden
                  }
                  img:first-child {
                    position:absolute;
                    top: calc(50vh - 36px);
                    left: calc(50vw - 36px);
                  }
                  img:nth-child(2) {
                    width: 100%;
                    height: auto
                  }
                  a {
                    display: inline-block;
                  }
                  a:focus,
                  a:hover {
                    color: white;
                    outline: 1px dotted white;
                    outline-offset: -40px;
                  }
                </style>
                <a href='https://www.youtube-nocookie.com/embed/3HqPbHUSeU4?autoplay=1'>
                  <img src='data:image/png;base64,' alt='Play'>
                  <img src='https://i.ytimg.com/vi/3HqPbHUSeU4/maxresdefault.jpg' alt='Thumbnail'>
                </a>"
              frameBorder="0"
              allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
              allowFullScreen
              loading="lazy"
            ></iframe>
            <h3>Donna</h3>
          </div>

          {/* Larry */}
          <div className={videoStyles.responsiveContainer}>
            <iframe
              className={videoStyles.responsiveIframe}
              title="Project Airtime | Larry"
              type="text/html"
              width="560"
              height="315"
              src="https://www.youtube-nocookie.com/embed/VzrlexXcEtM"
              srcDoc="
                <style>
                  * {
                    padding: 0;
                    margin: 0;
                    overflow: hidden
                  }
                  img:first-child {
                    position:absolute;
                    top: calc(50vh - 36px);
                    left: calc(50vw - 36px);
                  }
                  img:nth-child(2) {
                    width: 100%;
                    height: auto
                  }
                  a {
                    display: inline-block;
                  }
                  a:focus,
                  a:hover {
                    color: white;
                    outline: 1px dotted white;
                    outline-offset: -40px;
                  }
                </style>
                <a href='https://www.youtube-nocookie.com/embed/VzrlexXcEtM?autoplay=1'>
                  <img src='data:image/png;base64,' alt='Play'>
                  <img src='https://i.ytimg.com/vi/VzrlexXcEtM/maxresdefault.jpg' alt='Thumbnail'>
                </a>"
              frameBorder="0"
              allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
              allowFullScreen
              loading="lazy"
            ></iframe>
            <h3>Larry</h3>
          </div>
        </div>
      </Container>
    </section>
  )
}
